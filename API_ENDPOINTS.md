...
✅ OK  
**Regras:** criação e aprovação bloqueiam mesma pessoa como substituto, datas/dias inválidos, motivo `Outro` sem descrição, motivos de afastamento/atestado `Atestado Medico`, `INSS` e `INSS Indeterminado` sem descrição, `Extra` legado sem resposta de plantão extra e sobreposição com substituições pendentes/aprovadas. Ao editar uma movimentação aprovada, ela volta para `Pendente`, limpa os dados de aprovação e desativa vigências antigas até nova aprovação. `Atestado Medico` inicia com o prazo padrão sugerido, mas permite alterar a quantidade de dias e pode incluir CID opcional gravado em `motivoOutro`; o CID nunca é obrigatório. `INSS` tem quantidade de dias manual, sem prazo fixo de 15 dias. `plantaoExtra` é exibido na UI como `Sim | Nao | ''`, mas persiste em `plantao_extra` como `BOOLEAN` (`Sim` → `true`; `Nao`/vazio → `false`). A tela de criação não oferece mais o tipo `Extra`: plantões extras de atestado são registrados dentro de `Afastamento/Atestados`, com cada dia em `cadeia_substituicao` como `{ tipo: "extra", dataPlantao, funcionarioId, substitutoId, cargoExercido, equipePlantao }`, descrição em `motivoOutro` e `plantaoExtra = Sim`. Se a pessoa afastada estiver cobrindo férias por uma vigência ativa, `equipePlantao` e `cargoExercido` usam a equipe/função efetiva do dia; isso cobre feristas atuando em outras equipes. `INSS Indeterminado` usa `dataFim = 9999-12-31` como sentinela técnica e o campo `dias` representa somente a janela inicial de extras, podendo ser `0`; quando `dias = 0`, a substituição fixa começa na própria `dataInicio`; quando `dias > 0`, os extras são aplicados nos plantões da janela inicial e a substituição fixa começa no dia seguinte ao fim dessa janela. A troca de substituto de um `INSS Indeterminado` ativo cria uma nova movimentação `Pendente`; somente ao aprovar essa nova movimentação o sistema encerra a anterior na véspera da nova `dataInicio`, reprocessa sua vigência histórica e cria a nova vigência aprovada. `Substituição` aprovada gera vigência direta em `vigencia_substituicoes` com origem `substituicao`, fazendo o substituto herdar cargo/equipe do substituído durante o período. `INSS Indeterminado` aprovado também gera vigência direta em `vigencia_substituicoes` com origem `afastamento`, usando o substituto principal e a data inicial calculada após os extras. `Afastamento/Atestados` com extras e prazo definido não gera corrente nem vigência automática; a tela de funcionários deriva o status `Afastado` de afastamentos aprovados e vigentes, e a Escala Diária aplica os extras por data e equipe efetiva.
...
# 7.1. PTR-BA atual — `ptrbaCompletoService.ts`
...
```json
{
  "id": "uuid",
  "marcaAgente": "string",
  "produto": "LGE | Pó Químico Seco | Nitrogênio",
  "tipo": "Agente Extintor Principal | Agente Extintor Complementar | Agente Propelente",
  "dosagem": "1% | 3% | 6%",
  "classe": "AV | AR | HC",
  "quantidade": 0,
  ...
  "validadeEnsaioFogo": "string (ISO date)",
  "fabricacao": "string (ISO date)",
  "composicao": "MONOFOSFATO DE AMÔNIA | BICARBONATO DE SÓDIO",
  "testeHidrostatico": "string (ISO date)",
  "validadeTesteHidrostatico": "string (ISO date)",
  "validadeCilindro": "string (ISO date)",
  "localizacao": "string (legado/opcional)",
  "status": "Disponível | Baixo estoque | Vencido | Em manutenção | Fora de uso",
  "observacoes": "string",
  "createdBy": "string",
  "createdAt": "string",
  "updatedAt": "string"
